describe "filters", ->
  ...
  ddescribe "jsonDocument", ->
    ...
    it "can highlight strings", ->
      o = { foo: "bar" }
      expect(filter(o)).toContain('<span class="key">foo:</span> <span class="string">"bar"</span>')

    it "can highlight numbers", ->
      o = { bar: 123.99 }
      expect(filter(o)).toContain('<span class="key">bar:</span> <span class="number">123.99</span>')

    it "can highlight booleans", ->
      o = { foo: false }
      expect(filter(o)).toContain('<span class="key">foo:</span> <span class="boolean">false</span>')
